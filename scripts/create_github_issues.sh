...
create_labels_if_needed() {
    ...
    for label in "${labels[@]}"; do
        ...
        if ! gh label list 2>/dev/null | grep -q "$label"; then
            ...
            gh label create "$label" --color "FF0000" --description "EOL Alert" 2>/dev/null || echo "Warning: Could not create label $label"
        fi
    done
}

# Read the latest JSON report
LATEST_JSON=$(ls -t data/output/reports/*.json 2>/dev/null | head -n1)
...
gh auth status 2>&1 || {
    echo "GitHub CLI not authenticated. Trying to authenticate with GH_TOKEN..."
    if [ -n "$GH_TOKEN" ]; then
        echo "$GH_TOKEN" | gh auth login --with-token 2>/dev/null || echo "Failed to authenticate with GH_TOKEN"
    else
...
CRITICAL_TOOLS=$(jq -r '.tools[] | select(.criticality == "high") | "\(.tool_name)|\(.current_version)|\(.eol_status)|\(.eol_date)|\(.latest_version)"' "$LATEST_JSON" 2>/dev/null || echo "")
WARNING_TOOLS=$(jq -r '.tools[] | select(.criticality == "medium") | "\(.tool_name)|\(.current_version)|\(.eol_status)|\(.eol_date)|\(.latest_version)"' "$LATEST_JSON" 2>/dev/null || echo "")
...
This alert was automatically generated by the EOL Check workflow. Please address this issue promptly to maintain security and compatibility."
        ...
        # Create the issue (debug version)
        echo "Creating issue for: $tool_name $current_version"

        # Save issue body to temporary file to avoid shell quoting issues
        TEMP_FILE=$(mktemp)
        echo "$issue_body" > "$TEMP_FILE"

        # Try to create the issue with detailed error output
        if gh issue create --title "$issue_title" --body-file "$TEMP_FILE" 2>&1; then
            echo "Successfully created issue for $tool_name $current_version"
        else
            echo "Failed to create issue for $tool_name $current_version"
            # Try without body file as fallback
            echo "Trying fallback method..."
            gh issue create --title "$issue_title" --body "EOL Alert for $tool_name $current_version. Status: $eol_status. EOL Date: $eol_date. Latest: $latest_version." 2>/dev/null || \
            echo "Complete failure for $tool_name"
        fi
...
if [ -n "$CRITICAL_TOOLS" ]; then
    ...
    echo "No critical tools found"
fi

# Process warning tools
if [ -n "$WARNING_TOOLS" ]; then
